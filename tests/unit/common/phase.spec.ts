...
describe("PhaseModule interface", () => {
	it("allows implementing a phase module with prepare()", async () => {
		const mockModule: PhaseModule = {
			prepare: async (_ctx: PhaseContext): Promise<PhaseResult> => {
				return { success: true, retry: false };
			},
		};
		const result = await mockModule.prepare({} as PhaseContext);
