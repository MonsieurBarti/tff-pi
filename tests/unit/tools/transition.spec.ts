...
describe("handleTransition", () => {
	...
	it("auto-advances to next status when targetStatus omitted", () => {
		...
		expect(result.content[0]!.text).toContain("created → discussing");
		expect(getSlice(db, sliceId)!.status).toBe("discussing");
	});
	...
	it("transitions to explicit valid targetStatus", () => {
		...
		expect(result.isError).toBeUndefined();
		expect(result.content[0]!.text).toContain("discussing → researching");
	});
	...
	it("returns error for disallowed transition", () => {
		...
		expect(result.content[0]!.text).toContain("Allowed from 'created': discussing");
	});
	...
	it("returns error when no next status from closed", () => {
		...
		const result = handleTransition(db, sliceId);
		expect(result.isError).toBe(true);
		expect(result.content[0]!.text).toContain("No valid next status");
	});
});
