...
describe("queryState overview", () => {
	...
	it("returns project and all milestones", () => {
		...
		expect(result.project!.name).toBe("TFF");
		expect(result.milestones).toHaveLength(2);
		expect(result.milestones[0]!.name).toBe("Foundation");
		expect(result.milestones[1]!.name).toBe("Core");
	});
...
describe("queryState milestone", () => {
	...
	it("returns milestone and its slices", () => {
		...
		expect(result.milestone!.name).toBe("Foundation");
		expect(result.slices).toHaveLength(2);
		expect(result.slices[0]!.title).toBe("Auth");
		expect(result.slices[1]!.title).toBe("DB");
	});
...
describe("queryState slice", () => {
	...
	it("returns slice and its tasks", () => {
		...
		expect(result.slice!.title).toBe("Auth");
		expect(result.tasks).toHaveLength(2);
		expect(result.tasks[0]!.title).toBe("User entity");
		expect(result.tasks[1]!.title).toBe("Auth service");
	});
	...
	it("returns slice with tasks and dependencies", () => {
		...
		const t1Id = tasks[0]!.id;
		const t2Id = tasks[1]!.id;
		const t3Id = tasks[2]!.id;
